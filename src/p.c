...
Z T neg(){R(' '==*ts.c||'('==*ts.c)?mt(HI):num();}        //<! - token. NUM if -[0-9]
...
T nt(){ws();ts.s=ts.c;C c=nc();                           //<! return next token
 ...
 CS('*', R mt(ST))  CS('+', R mt(PL))  CS('%', R mt(DV))  CS('!', R mt(BA))  CS('-', R neg())   CS('@', R mt(AT))  
...
// parser utilities
I ngp(I x){R -(x);} I fact(T *tk){TT t=tk->t;R NUM==t?strtod(tk->s,NULL):LP==t?pr(tk+1):ID==t?(vt[*tk->s-'a']):-128;} //<! parse factor (NUM/parens)
I pr(T *tk){  //<! parse
 if(HI==tk->t)R ngp(pr(tk+1));if(END==tk[1].t||RP==tk[1].t)R fact(tk);I y,i=0; //<! if next token is END or ), evaluate+return current token
