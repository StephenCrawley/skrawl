...
ZT hy(){R((NUM==ts.bp[-1].t||RP==ts.bp[-1].t)&&(' '-ts.s[-1]||' '==*ts.c))?mt(HY):cn(*ts.c)?num():mt(HY);} //<! handle minus/unary neg/negative num
...
T nt(){ws();ts.s=ts.c;C c=nc();                                      //<! return next token
 ...
 CS('*',R mt(ST)) CS('+',R mt(PL)) CS('%',R mt(DV)) CS('!',R mt(BA)) CS('-',R hy())   CS('.', R mt(DT)) CS('\'',R mt(AP)) 
...
I pr(T *tk){ //<! parse
 if(END==tk->t)R 0;if(HY==tk->t)R neg(pr(tk+1));if(TL==tk->t)R not(pr(tk+1));if(END==tk[1].t||RP==tk[1].t)R fact(tk);I y,i=0; //<! neg,not,if next token is END or )->eval+return current token
 ...
 I x=fact(tk);if(DBGP)O("x=%d, y=%d\n",x,y);switch(tk[i+1].t){CS(PL,R x+y)CS(ST,R x*y)CS(HY,R x-y)CS(EQ,R x==y)CS(RA,R x>y)CS(CL,R vt[*tk->s-'a']=y)}; //<! case +*-:~
 R -128;}