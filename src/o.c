...
K kgn(J n){K k=ma(SZ(K),n);xT(k)=KK;R k;}                                                     // return generic object of length n
...
K k(I t,J n){R KK==t?kgn(n):KJ==t?kjn(n):-KJ==t?kj():KF==t?kfn(n):-KF==t?kf():KN==t?knl():KQ==t?kqt():E_NYI;}
// str to K
...
// refcount
V r0(K x){if(KK==xt){DO(xn,r0(xG(x)[i]))}xr--;if(1>xr)free(x);}
V r1(K x){if(KK==xt){DO(xn,r1(xG(x)[i]))}xr++;}
...
ZV pk_(K x){if(0<xt&&1==xn)O(",");if(KK==xt){O("(");DO(xn,pk_(xG(x)[i]);if(i!=xn-1)O(";"));O(")");}else if(KJ==ABS(xt)){pj(x);}else if(KF==ABS(xt)){pf(x);}
