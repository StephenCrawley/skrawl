...
K k(I t,J n){R KG==t?kgn(n):KJ==t?kjn(n):-KJ==t?kj():KF==t?kfn(n):-KF==t?kf():E_NYI;}
// str to K
...
V pk(K x){
 ...
 if(KG==xt){DO(xn,pk_(xG(x)[i]);if(i!=xn-1)O("\n"))}else if(KJ==ABS(xt)){pj(x);}else if(KF==ABS(xt)){pf(x);}else if(err(x)){O("%s",xC(x));}else{pk(E_NYI);};O("\n");r0(x);}