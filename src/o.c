...
K sqz(K x){P(KK!=xt,x);C t=xT(xK(x)[xn-1]);K z;P(0<=t&&1==xn,(z=xK(x)[0],r1(z),r0(x),z));P(0<=t,x);G b=1;DO(xn-1,b=(b&&t==xT(xK(x)[i]))); // squeeze. eg (1;2;3)->1 2 3 N.B. assumes child items are atoms
 P(!b,x);if(-KJ==t){z=k(KJ,xn);DO(xn,xJ(z)[i]=*xJ(xK(x)[i]));}else{z=k(KF,xn);DO(xn,xF(z)[i]=*xF(xK(x)[i]));};
 r0(x);R z;}
// str to K
...
K r1(K x){if(KK==xt){DO(xn,r1(xK(x)[i]))}xr++;R x;}
...
ZV pk_(K x){if(0<xt&&1==xn)O(",");if(KK==xt){if(1==xn){O(",");}else{O("(");}DO(xn,pk_(xK(x)[i]);if(i!=xn-1)O(";"));if(1!=xn)O(")");}else if(KJ==ABS(xt)){pj(x);}else if(KF==ABS(xt)){pf(x);}
