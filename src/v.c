...
K enl(K x){K z;if(0<=xt){z=k(KG,1);xG(z)[0]=x;r1(x);}else if(-KJ==xt){z=k(KJ,1);xJ(z)[0]=xJ(x)[0];}else if(-KF==xt){z=k(KF,1);xF(z)[0]=xF(x)[0];}else{z=E_NYI;}r0(x);R z;}
// dyad
...
K cat(K x,K y){K z=ABS(xt)==ABS(yt)?k(ABS(xt),xn+yn):k(KG,xn+yn);J j=0;
 ...
 else                   {DO(xn,xG(z)[j]=KG==ABS(xt)?(r1(xG(x)[i]),xG(x)[i]):KJ==ABS(xt)?kjx(xJ(x)[i]):kfx(xF(x)[i]);++j);
                         DO(yn,xG(z)[j]=KG==ABS(yt)?(r1(xG(y)[i]),xG(y)[i]):KJ==ABS(yt)?kjx(xJ(y)[i]):kfx(xF(y)[i]);++j)} //inefficient
 r0(x);r0(y);R z;}
